...
public class PolymathDelegationService : IPolymathDelegationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning(
                "Requested {Requested} agents, capped to {Max} (cost guardrail)",
        ...
        if (effectiveCount == 1)
        {
            return await SingleAgentFallback(query, cancellationToken);
        }

        _logger.LogInformation(
            "Starting polymath delegation: {AgentCount} agents, strategy={Strategy}",
            effectiveCount, strategy);
        ...
        // Synthesize results
        var synthesized = await Synthesize(responses.ToList(), query, strategy, cancellationToken);
    ...
    private async Task<string> Synthesize(
        List<AgentResponse> responses, string originalQuery,
        SynthesisStrategy strategy, CancellationToken cancellationToken)
    {
        return strategy switch
        {
            SynthesisStrategy.Best => SelectBest(responses),
            SynthesisStrategy.Consensus => await SynthesizeConsensus(responses, originalQuery, cancellationToken),
            SynthesisStrategy.Merge => await SynthesizeMerge(responses, originalQuery, cancellationToken),
            _ => responses.First().Response
        };
    }
        ...
        List<AgentResponse> responses, string originalQuery, CancellationToken cancellationToken)
    {
        ...
        var response = await _llmClient.GetResponse(
            ...
            toolsContext: null, images: null, cancel: cancellationToken);
        ...
        string query, CancellationToken cancellationToken)
    {
        ...
        var agentResponse = await QueryAgent(0, Perspectives[0], query, cancellationToken);
