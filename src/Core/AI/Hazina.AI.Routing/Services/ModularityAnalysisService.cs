...
public partial class ModularityAnalysisService : IModularityAnalysisService
{
    ...
    public double CalculateScore(string taskDescription)
    {
        ...
        var parallelHits = ParallelIndicators.Count(indicator => lower.Contains(indicator, StringComparison.Ordinal));
        score += parallelHits * 0.08;

        // Count sequential indicators
        var sequentialHits = SequentialIndicators.Count(indicator => lower.Contains(indicator, StringComparison.Ordinal));
