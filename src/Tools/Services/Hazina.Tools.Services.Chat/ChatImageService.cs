        ...
        private async Task<ChatConversation> GenerateImageInternal(string projectId, string chatId, Project project, GeneratorMessage chatMessage, CancellationToken cancel, string? userId, bool isImageSet)
        {
            ...
            try
            {
                ...
                if (_imageAnalysisService != null)
                {
                    try
                    {
                        Console.WriteLine($"[ChatImageService] Starting AI analysis for {fileName}...");
                        var analysisResult = await _imageAnalysisService.AnalyzeImageAsync(
                            imageBytes: resolved.Data,
                            context: $"This image was generated with the prompt: {prompt}",
                            cancel: cancel
                        );
                        aiDescription = analysisResult.Description;
                        aiTags = analysisResult.Tags;
                        Console.WriteLine($"[ChatImageService] AI Analysis complete. Description: {aiDescription}");
                        Console.WriteLine($"[ChatImageService] AI Tags: {string.Join(", ", aiTags)}");
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"[ChatImageService] AI analysis failed: {ex.Message}. Using prompt-based tags.");
                    }
                }

                // Update uploadedFiles.json with AI analysis results
                if (_imageAnalysisService != null && (aiDescription != null || aiTags != null))
                {
                    try
                    {
                        var listFilePath = Path.Combine(FileLocator.GetProjectFolder(projectId), "uploadedFiles.json");
                        var uploadedFiles = await FileHelper.GetUploadedFilesListAsync(listFilePath);
                        var uploadedFile = uploadedFiles.FirstOrDefault(f => f.Filename == fileName);
                        if (uploadedFile != null)
                        {
                            uploadedFile.Description = aiDescription;
                            uploadedFile.Tags = aiTags ?? uploadedFile.Tags;
                            uploadedFile.Tags.Add("generated");  // Ensure "generated" tag is present
                            uploadedFile.Tags = uploadedFile.Tags.Distinct().ToList();  // Remove duplicates
                            await FileHelper.UpdateUploadedFilesListAsync(listFilePath, uploadedFile);
                            Console.WriteLine($"[ChatImageService] Updated uploadedFiles.json with AI metadata");
                        }
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"[ChatImageService] Failed to update uploadedFiles with AI metadata: {ex.Message}");
                    }
                }
                var metadata = new GeneratedImageInfo
                {
                    ...
                    Tags = aiTags ?? DetermineImageTags(prompt),
